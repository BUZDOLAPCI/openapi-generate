 ...
 */
export function generateToolSchemas(parsedSpec) {
    try {
 ...
 */
function operationToTool(operation, schemas) {
    const properties = {};
    const required = [];
    ...
    if (operation.request_body) {
        ...
        const contentType = operation.request_body.content['application/json'] ||
 ...
 */
function convertSchemaToJSONSchema(schema, allSchemas) {
    // Handle $ref
    if (schema.$ref) {
 ...
 */
function buildToolDescription(operation) {
    const parts = [];
