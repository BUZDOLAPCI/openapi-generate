 ...
 */
export function generateServerScaffold(parsedSpec, language, options = {}) {
    try {
 ...
 */
function generateTypeScriptScaffold(spec, tools, config) {
    const files = [];
    // package.json
    files.push({
        path: 'package.json',
 ...
 */
function generatePythonScaffold(spec, tools, config) {
    const files = [];
    // pyproject.toml
    files.push({
        path: 'pyproject.toml',
...
function generateConfigFile(serverName, serverVersion) {
...
function generateToolFile(tool, baseUrl) {
...
function generateServerFile(toolNames, serverName, serverVersion) {
...
function generateReadmeFile(serverName, description, toolCount, language = 'typescript') {
    if (language === 'python') {
        return `# ${serverName}
...
function generateUnitTestFile(toolNames) {
...
function generatePythonMainFile(serverName) {
...
function generatePythonServerFile(tools, serverName) {
...
${tools
        .map((tool) => `        if name == "${tool.name.replace(/-/g, '_')}":
...
function generatePythonTypesFile() {
...
function generatePythonConfigFile(serverName, serverVersion, baseUrl) {
...
function generatePythonToolFile(tool, baseUrl) {
...
function generatePythonTestFile(toolNames) {
