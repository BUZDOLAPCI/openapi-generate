...
export function createServer(
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
      case 'openapi_parse': {
        const specUrlOrJson = (args as { spec_url_or_json: string }).spec_url_or_json;
        if (!specUrlOrJson) {
          return {
            content: [
              {
      ...
      case 'generate_tool_schemas': {
        const parsedSpec = (args as { parsed_spec: ParsedOpenAPISpec }).parsed_spec;
        if (!parsedSpec) {
          return {
            content: [
              {
